...
[ -f "$ORIENTDB_HOME"/lib/orientdb-tools-3.2.34.jar ] || ORIENTDB_HOME=`cd "$PRGDIR/.." ; pwd`
...
exec "$JAVA" -client $JAVA_OPTS $ORIENTDB_OPTS_MEMORY $ORIENTDB_SETTINGS $SSL_OPTS \
    -Dfile.encoding=utf-8 -Dorientdb.build.number="UNKNOWN@r${buildNumber}; 2024-09-30 17:10:43+0000" \
    -cp "$ORIENTDB_HOME/lib/orientdb-tools-3.2.34.jar:$ORIENTDB_HOME/lib/*:$ORIENTDB_HOME/plugins/*" \
